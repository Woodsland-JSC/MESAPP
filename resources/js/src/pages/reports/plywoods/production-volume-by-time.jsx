...
const STAGE_COLORS = {
    'XV': '#ea7ccc',    // Xếp ván - Hồng
    'TV': '#673ab7',    // Tạo ván - Tím
    'LL': '#cddc39',    // Lên lớp - Vàng xanh
    'TV2': '#ffc003',   // Tạo ván 2 - Vàng
    'HTV': '#4caf50',   // Hoàn thiện ván - Xanh lá
...
function ProductionVolumeByTimeReport() {
    ...
    const aggregateItemsByWeek = (data) => {
        ...
        return stageOrder ? Array.from(itemMap.values()).sort((a, b) => {
    ...
    useEffect(() => {
        if (dailyData && dailyData.length > 0 && stages && stages.length > 0) {
            ...
            stageResults['no_stage'] = Array(dates.length).fill(0);
            ...
            res.forEach(item => {
                ...
                if (dateIndex !== -1) {
                    ...
                    // Kiểm tra nếu item có stage và stage đó tồn tại trong stageResults
                    if (item.stage && stageResults[item.stage]) {
                        stageResults[item.stage][dateIndex] += value;
                    } else if (!item.stage || item.stage === '') {
                        // Nếu không có stage, thêm vào no_stage
                        stageResults['no_stage'][dateIndex] += value;
                    }
                }
            });
    ...
    const data = useMemo(() => {
        ...
        const calculateWeeklyData = (stage, unit) => {
            return Array.from({ length: 53 }, (_, weekIndex) => {
                ...
                return rowData
                    .filter(row => {
                        // Nếu stage là null, lấy những row không có stage
                        if (stage === null) {
                            return !row.stage || row.stage === '';
                        }
                        // Ngược lại, lấy theo stage bình thường
        ...
        const getDataForStage = (stageName) => {
            switch (selectedTimeRange) {
                case 'week':
                    return calculateWeeklyData(stageName, selectedUnit);
                case 'month':
                    return calculateMonthlyData(stageName, selectedUnit);
                case 'day':
                    if (stageName === null) {
                        return dailyChartData['no_stage'] || [];
                    }
                    return dailyChartData[stageName] || [];
                default:
                    return [];
            }
        };
        ...
        const datasets = stages
            ...
            .map((stage, index) => ({
                label: stage.label,
                backgroundColor: getStageColor(stage.value, index),
                data: getDataForStage(stage.label),
            }));
        ...
        return {
            ...
            scales: {
                ...
                y: {
                    title: {
                        display: true,
                        text: selectedUnit == 'm3' ? 'M3' : 'Sản lượng',
                        font: {
                            family: 'Lexend Variable,Source Serif Pro,sans-serif',
                            size: 14,
                        },
