...
class OITWService
{
    ...
    private $SQL_FIND_ITEM_PRINT_BY_WH = <<<SQL
        SELECT 
            A."ItemCode", 
            B."ItemName", 
            A."WhsCode", 
    ...
    public function findItemByWh($whCode, $code)
    {
        try {
            return $this->hanaService->select($this->SQL_FIND_ITEM_PRINT_BY_WH, [$whCode, $code, $code]);
        } catch (Exception $e) {
            throw new Exception("Tìm item theo mã có lỗi");
        }
    }
}
