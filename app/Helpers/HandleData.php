...
if (!function_exists('playloadIssueCBG')) {
    function playloadIssueCBG($ItemCode, $Qty, $WarehouseCode, $branch, $LSX, $TO, $NGIAO, $NNHAN, $QCNUMBER,$id)
    {
        ...
        // dd($ItemCode, $WarehouseCode);
        // Fetch the single record
        $row = odbc_fetch_array($stmt);

        if ($row === false) {
            throw new \App\Exceptions\CustomException('No data returned.');
        }
        ...
        $stmt = odbc_prepare($conDB, $query);
        if (!$stmt) {
            throw new \App\Exceptions\CustomException('Error preparing SQL statement: ' . odbc_errormsg($conDB));
        }
        if (!odbc_execute($stmt, [$ItemCode, $WarehouseCode, $Qty, (int)$itemType])) {
            throw new \App\Exceptions\CustomException('Error executing SQL statement: ' . odbc_errormsg($conDB));
        }
        $results = array();
